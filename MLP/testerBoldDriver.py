...
#To keep track of what the previous parameter states were
...
class Node:
    ...
    def updateBias(self):
        ...
        #To keep track of what previous bias values are
        for bias in startingBiasList:
            if bias == self.bias:
                startingBiasList[startingBiasList.index(bias)] = self.bias
...
class Weight():
    ...
    def updateWeights(self):
        ...
        #To keep track of what previous weight values were 
        for weight in startingWeightList:
            if weight == self.weightValue:
                startingWeightList[startingWeightList.index(weight)] = self.weightValue
...
def BoldDriver():
    ...
    for node in nodeList:
        if isinstance(node, OutputNode):
            #Too flat
            if node.delta > 0.04:
                if learningParameter * 0.7 > 0.0001:
                    learningParameter = learningParameter * 0.7
                return False
            #Steep enough
            else:
                if learningParameter * 1.05 < 0.005:
                    learningParameter = learningParameter * 1.05
                return True
...
def main1():
    ...
    new_sheet_obj = write_wb_obj["BoldDriverOutliers"]

    for y in range (1, len(errorList) + 1):
        new_sheet_obj.cell(row = 1, column = y).value = errorList[y - 1]
