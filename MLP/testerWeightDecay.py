...
class OutputNode(Node):
    ...
    def updateDelta(self):
        fDash = self.uValue * (1 - self.uValue)
        self.delta = (self.actualValue - self.uValue + v * omega) * fDash
...
class Weight():
    ...
    def updateWeights(self):
        ...
        weightChange = learningParameter * endDelta * startValue
...
def main1():
    ...
    parameterList = []
    for node in nodeList:
        if isinstance(node, OutputNode) or isinstance(node, HiddenNode):
            parameterList.append(node.bias)
    for x in range(1, 101):
        ...
        for entry in entryList:
            ...
            try:
                ...
                break
            v = 1/(learningParameter * x)
            omega = 1/(2*len(parameterList))
            intermediate = 0
    ...
    new_sheet_obj = write_wb_obj["WeightDecay"]
